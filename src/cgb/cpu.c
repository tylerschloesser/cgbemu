...
#define VBLANK_CYCLES CLOCK_SPEED / 60
...
static EmulatorState emulator_state = INVALID;
...
void cpu_emulate() {
	...
	while( emulator_state != STOPPED ) {
		...
		if(cycles > VBLANK_CYCLES) {
			...
			int sleep_usec = (1000000 - get_elapsed_usec(time_start, time_end) - 1)/60;
			usleep(sleep_usec);
			...
			// if the cpu has stopped running, don't service interrupts
			// ********not really sure if this will happen...
			if( emulator_state != STARTED ) {
				fprintf( stdout, "skipping interrupts\n" );
				continue;
			}
		}
